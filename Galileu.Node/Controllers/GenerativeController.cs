...
public class GenerativeController : ControllerBase
{
    ...
    [HttpPost("trainer/hybrid")]
    public async Task<IActionResult> HybridTrainer([FromBody] Trainer trainerOptions)
    {
        try
        {
            // Chame o novo método no serviço
            await _generativeService.TrainWithTeacherAsync(trainerOptions);
            return Ok("O treinamento híbrido com professor de IA foi iniciado.");
        }
        catch (Exception ex)
        {
            return StatusCode(500, new { message = $"Um erro ocorreu: {ex.Message}" });
