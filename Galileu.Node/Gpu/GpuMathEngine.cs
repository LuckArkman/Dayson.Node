    ...
    int t)                          // Timestep
{
    ...
    {
        var tensor = new GpuTensor(shape, _context, _commandQueue);
        GC.KeepAlive(tensor);
        return tensor;
    }
    public IMathTensor CreateTensor(float[] data, int[] shape)
    {
        // 🧠 Cópia isolada — evita referência ao array original
        float[] buffer = new float[data.Length];
        Buffer.BlockCopy(data, 0, buffer, 0, data.Length * sizeof(float));

        // Cria o tensor a partir da cópia interna
        var tensor = new GpuTensor(buffer, shape, _context, _commandQueue);

        // 🔥 Libera referência do array original imediatamente
